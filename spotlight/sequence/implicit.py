...
class ImplicitSequenceModel(object):
    ...
    def _get_validation_loss(self, sequences, weights):
        with torch.no_grad():
            sequences, weights = shuffle(sequences, weights, random_state=self._random_state)
            sequences_tensor = gpu(torch.from_numpy(sequences), self._use_cuda).detach()
            weights_tensor = gpu(torch.from_numpy(weights), self._use_cuda).detach()

            epoch_loss = 0

            for minibatch_num, (batch_sequence, batch_weights) in enumerate(minibatch(sequences_tensor, weights_tensor, batch_size=self._batch_size)):
                ...
                positive_prediction = self._net(user_representation, sequence_var).detach()

                if self._loss == 'adaptive_hinge':
                    negative_prediction = self._get_multiple_negative_predictions(sequence_var.size(), user_representation, n=self._num_negative_samples).detach()
                else:
                    negative_prediction = self._get_negative_prediction(sequence_var.size(), user_representation).detach()

                loss = self._loss_func(positive_prediction, negative_prediction, mask=(sequence_var != PADDING_IDX), weights=weights_var).detach()
    ...
    def fit(self, interactions, validation_interactions=None, verbose=False):
        ...
        if validation_interactions is not None:
            validation_sequences = validation_interactions.sequences.astype(np.int64)
            validation_weights = validation_interactions.weights.astype(np.float32)
        ...
        for epoch_num in range(self._n_iter):
            ...
            for minibatch_num, (batch_sequence, batch_weights) in enumerate(minibatch(sequences_tensor, weights_tensor, batch_size=self._batch_size)):
                ...
                loss = self._loss_func(positive_prediction, negative_prediction, mask=(sequence_var != PADDING_IDX), weights=weights_var)
                epoch_loss += loss.item()
            ...
            if validation_interactions is not None:
                val_loss[epoch_num] = self._get_validation_loss(validation_sequences, validation_weights)

            if verbose and validation_interactions is not None:
                print('Epoch {}: loss {} \t val-loss {}'.format(epoch_num, train_loss[epoch_num], val_loss[epoch_num]))
            else:
                print('Epoch {}: loss {}'.format(epoch_num, train_loss[epoch_num]))
