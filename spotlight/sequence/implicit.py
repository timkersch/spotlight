...
class ImplicitSequenceModel(object):
	...
	def _get_validation_loss(self, sequences):
		with torch.no_grad():
			...
			for minibatch_num, batch_sequence in enumerate(minibatch(sequences_tensor, batch_size=self._batch_size)):
				...
				loss = self._loss_func(positive_prediction, negative_prediction, mask=(sequence_var != PADDING_IDX)).detach()
	...
	def fit(self, interactions, validation_interactions=None, verbose=False):
		...
		for epoch_num in range(self._n_iter):
			...
			if validation_interactions is not None:
				val_loss[epoch_num] = self._get_validation_loss(validation_sequences)
