...
def _sliding_window_w_time(items, timestamps, weights, window_size, step_size=1):

    for i in range(len(items), 0, -step_size):
        yield items[max(i - window_size, 0):i], timestamps[i-1], weights[i-1]


def _generate_sequences(user_ids, item_ids, timestamps, weights, indices, max_sequence_length, step_size):
    ...
    for i in range(len(indices)):
        ...
        for seq, timestamp, weight in _sliding_window_w_time(item_ids[start_idx:stop_idx], timestamps[start_idx:stop_idx], weights[start_idx:stop_idx], max_sequence_length, step_size):

            yield (user_ids[i], timestamp, weight, seq)
...
class Interactions(object):
    ...
    def __init__(self, user_ids, item_ids,
        ...
        if weights is None:
            self.weights = np.ones(self.user_ids.shape[0], dtype=np.float32)
        else:
...
class SequenceInteractions(object):
    ...
    def __init__(self,
                 sequences,
                 user_ids=None, num_items=None, timestamps=None, weights=None):
