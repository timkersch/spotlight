...
def _sliding_window_w_time(items, timestamps, window_size, step_size=1):

	for i in range(len(items), 0, -step_size):
		yield items[max(i - window_size, 0):i], timestamps[i-1]


def _generate_sequences(user_ids, item_ids, timestamps, indices, max_sequence_length, step_size):
	...
	for i in range(len(indices)):
		...
		for seq, timestamp in _sliding_window_w_time(item_ids[start_idx:stop_idx], timestamps[start_idx:stop_idx], max_sequence_length, step_size):

			yield (user_ids[i], timestamp, seq)
...
class Interactions(object):
	...
	def to_sequence(self, max_sequence_length=10, min_sequence_length=None, step_size=None):
		...
		for i, (uid, timestamp, seq) in enumerate(_generate_sequences(user_ids, item_ids, timestamps, indices, max_sequence_length, step_size)):
