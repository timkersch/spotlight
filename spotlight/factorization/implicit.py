...
class ImplicitFactorizationModel(object):
    ...
    def _get_validation_loss(self, interactions):
        with torch.no_grad():
            ...
            users, items, weights = shuffle(user_ids, item_ids, weights, random_state=self._random_state)

            user_ids_tensor = gpu(torch.from_numpy(users), self._use_cuda).detach()
            item_ids_tensor = gpu(torch.from_numpy(items), self._use_cuda).detach()
            weights_tensor = gpu(torch.from_numpy(weights), self._use_cuda).detach()

            epoch_loss = 0

            for (minibatch_num, (batch_user, batch_item, batch_weight)) in enumerate(minibatch(user_ids_tensor, item_ids_tensor, weights_tensor, batch_size=self._batch_size)):
                positive_prediction = self._net(batch_user, batch_item).detach()

                if self._loss == 'adaptive_hinge':
                    negative_prediction = self._get_multiple_negative_predictions(batch_user, n=self._num_negative_samples).detach()
                else:
                    negative_prediction = self._get_negative_prediction(batch_user).detach()

                loss = self._loss_func(positive_prediction, negative_prediction, weights=batch_weight).detach()
    ...
    def fit(self, interactions, validation_interactions=None, verbose=False):
        ...
        for epoch_num in range(self._n_iter):
            ...
            for (minibatch_num,
                 (batch_user,
                  batch_item,
                  batch_weight)) in enumerate(minibatch(user_ids_tensor,
                                                        item_ids_tensor,
                                                        weights_tensor,
                ...
                loss = self._loss_func(positive_prediction, negative_prediction, weights=batch_weight)
                epoch_loss += loss.item()
            ...
            if validation_interactions is not None:
                val_loss[epoch_num] = self._get_validation_loss(validation_interactions)
